...
public class Lab61 {
    ...
    static int encontrarDuplicado(
            int[] arreglo, int elemento) { // funcion que detecta si hay un numero repetido
        int bandera = 0; // bandera que se activa si es un numero duplicado
        for (int i = 0;
                i < arreglo.length;
                i++) { // recorre el arreglo hasta encontrar el valor duplicado
            if (elemento
                    == arreglo[i]) { // si encuntra un valor duplicado, ejecuta la linea de abajo
